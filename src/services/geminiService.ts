...
/** Proxy seguro hacia /api/gemini (la API key vive en el servidor, no en el bundle). */
async function callGemini(
  model: string,
  contents: { parts: ({ text?: string; inlineData?: { mimeType: string; data: string } } | string)[] }[],
  ...
  if (!res.ok) {
    ...
    throw new Error(err?.error || 'Error al llamar a Gemini');
  }
