  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-2 sm:p-6 pb-24 w-full">
      <div className="w-full max-w-[1200px] mx-auto space-y-6">
        ...
        <PageHeader
          emoji="🥊"
          title="Guantes Para Encajarte"
          subtitle="Calendario de Sesiones"
          right={
            <div className="flex items-center gap-2">
              <button aria-label="Volver"
                onClick={() => navigate(-1)}
                className="p-3 bg-slate-900/50 hover:bg-slate-800 rounded-2xl transition-all btn-press"
              >
                <ArrowLeft className="w-5 h-5 text-slate-400" />
              </button>
              <button
                onClick={() => setShowMyBookings(!showMyBookings)}
                className="p-3 bg-slate-900/50 hover:bg-slate-800 rounded-2xl transition-all relative btn-press"
              >
                <CalendarIcon
                  className={cn(
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-[2.5rem] p-6 sm:p-8 space-y-6 backdrop-blur-md">
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <button
                ...
                className="px-6 py-3 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg shadow-primary/20 btn-press"
              >
            ...
            <div className="w-full p-8 sm:p-12 space-y-8">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[10px] font-black uppercase text-primary hover:underline btn-press"
                >
                  Volver a Reservar
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {dataLoading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="h-52 rounded-4xl skeleton" />
                  ))
                ) : bookings.length === 0 ? (
                  <motion.div
                    ...
                    transition={spring}
                    className="col-span-full py-20 flex flex-col items-center text-slate-600"
                  >
                    <motion.div
                    ...
                    <p className="text-sm font-black uppercase italic tracking-[0.3em] text-slate-500">
                  ...
                  bookings.map((b) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="mt-8 w-full py-3 bg-red-500/5 hover:bg-red-500 text-red-500 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all btn-press"
                      >
                        Cancelar Reserva
                      </button>
                    </motion.div>
                      ...
                      <div className="space-y-2">
                        ...
                        {editingField === 'description' ? (
                          <div className="space-y-2">
                            ...
                            />
                            <div className="flex gap-2">
                              <button type="button" onClick={handleSaveField} className="flex-1 py-2 bg-primary text-white rounded-lg text-xs font-black uppercase">Guardar</button>
                              <button type="button" onClick={() => setEditingField(null)} className="px-3 py-2 bg-slate-700 text-slate-300 rounded-lg text-xs font-bold">Cancelar</button>
                            </div>
                          </div>
                        ) : (
                      ...
                      <div className="space-y-2 pt-4 border-t border-slate-800/50">
                        ...
                        {editingField === 'rules' ? (
                          <div className="space-y-2">
                            ...
                            />
                            <div className="flex gap-2">
                              <button type="button" onClick={handleSaveField} className="flex-1 py-2 bg-emerald-600 text-white rounded-lg text-xs font-black uppercase">Guardar</button>
                              <button type="button" onClick={() => setEditingField(null)} className="px-3 py-2 bg-slate-700 text-slate-300 rounded-lg text-xs font-bold">Cancelar</button>
                            </div>
                          </div>
                        ) : (
                ...
                <div className="flex-1 space-y-8 animate-in fade-in duration-700">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button aria-label="Volver"
                        onClick={prevMonth}
                        className="p-2.5 hover:bg-slate-800 rounded-xl transition-all border border-slate-800 btn-press"
                      >
                        <ChevronLeft className="w-5 h-5 text-slate-400" />
                      </button>
                      <button aria-label="Avanzar"
                        onClick={nextMonth}
                        className="p-2.5 hover:bg-slate-800 rounded-xl transition-all border border-slate-800 btn-press"
                      >
                    ...
                    {dataLoading ? (
                      ...
                      >
                        ...
                        {calendarDays.map((day, i) => {
                          ...
                          if (!isCurrent) return <motion.div key={i} variants={staggerItem} className="aspect-square" />;
                ...
                <div className="w-full lg:w-72 space-y-8 animate-in slide-in-from-right-4 duration-500">
                  ...
                  {dataLoading ? (
                    ...
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={spring}
                      className="py-20 flex flex-col items-center justify-center bg-slate-950/20 rounded-3xl border border-slate-800/50"
                    >
                      <motion.div
                    ...
                    <motion.div
                      ...
                      initial="hidden"
                      animate="show"
                      className="space-y-3 max-h-[450px] overflow-y-auto pr-3 custom-scrollbar"
                    >
