    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-slate-900 dark:text-white pb-32 font-sans">
        ...
        <div className="fixed bottom-24 right-4 z-40 flex flex-col items-end gap-3">
          ...
          {pendingVideos.length > 0 && (
            <button
              ...
              className="btn-press flex items-center gap-2 bg-amber-500 text-white px-4 py-2.5 rounded-2xl shadow-xl shadow-amber-500/30 font-black text-[10px] uppercase tracking-widest hover:bg-amber-600 transition-all"
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="btn-press px-3 py-2 bg-amber-500/10 text-amber-500 rounded-xl border border-amber-500/20 text-[10px] font-black uppercase hover:bg-amber-500/20 transition-all"
            ...
            <button
              ...
              className="btn-press px-3 py-2 bg-emerald-500/10 text-emerald-500 rounded-xl border border-emerald-500/20 text-[10px] font-black uppercase hover:bg-emerald-500/20 transition-all flex items-center gap-1 disabled:opacity-50"
            ...
            <button
              onClick={() => setShowAdminPanel(true)}
              id="admin-panel-open"
              className="btn-press px-3 py-2 bg-blue-500/10 text-blue-500 rounded-xl border border-blue-500/20 text-[10px] font-black uppercase hover:bg-blue-500/20 transition-all"
            ...
            <button aria-label="Agregar"
              ...
              id="add-video-btn"
              className="btn-press p-4 bg-primary text-white rounded-full shadow-2xl shadow-primary/30 hover:scale-110 active:scale-95 transition-all border-4 border-white dark:border-slate-900"
            >
        ...
        <form onSubmit={handleSubmitVideo} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={handleCloseUploadModal} disabled={isUploading}
              className="btn-press flex-1 py-3 rounded-2xl text-sm font-black uppercase text-slate-500 hover:bg-slate-100 transition-all disabled:opacity-50">
              Cancelar
            </button>
            <button type="submit" disabled={isUploading}
              className="btn-press flex-[2] bg-primary text-white py-3 rounded-2xl text-sm font-black uppercase shadow-xl shadow-primary/30 disabled:opacity-50 transition-all">
      ...
      <header className="px-4 sm:px-6 py-6 sm:py-8">
        <PageHeader
          ...
          subtitle={`${filteredVideos.length} ejercicios disponibles`}
          right={
            <div className="flex items-center gap-2">
              <button aria-label="Volver" type="button" onClick={() => navigate(-1)}
                className="btn-press p-2.5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 text-slate-500 hover:text-primary transition-all shadow-sm">
        ...
        <Reveal delay={0.05}>
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`btn-press relative flex items-center gap-2 px-4 py-3.5 rounded-2xl border font-black text-sm transition-all ${
                activeFilterCount > 0
            ...
            <button
              ...
              className="btn-press flex items-center justify-center p-3.5 rounded-2xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-500 hover:text-primary transition-all shadow-sm"
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              {filters.lugar && (
                <span className="flex items-center gap-1 px-3 py-1.5 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase">
                  ...
                  <button aria-label="Cerrar" type="button" onClick={() => setFilters({ ...filters, lugar: null })} className="ml-1">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              {filters.categoria && (
                <span className="flex items-center gap-1 px-3 py-1.5 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase">
                  ...
                  <button aria-label="Cerrar" type="button" onClick={() => setFilters({ ...filters, categoria: null })} className="ml-1">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              {filters.difficulty && (
                <span className="flex items-center gap-1 px-3 py-1.5 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase">
                  {filters.difficulty}
                  <button aria-label="Cerrar" type="button" onClick={() => setFilters({ ...filters, difficulty: null })} className="ml-1">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              )}
              <button
                ...
                className="btn-press px-3 py-1.5 text-[10px] font-black text-slate-400 hover:text-primary uppercase tracking-widest transition-colors"
              >
                Limpiar todo
              </button>
            </div>
        ...
        <Reveal delay={0.15}>
          <div className="flex gap-2 mt-4 overflow-x-auto pb-1 scrollbar-hide">
            <button
              ...
              className={`btn-press whitespace-nowrap px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                !filters.categoria
            ...
            {isAdmin && (
              showAddCategory ? (
                <form onSubmit={handleAddCategory} className="flex items-center gap-2 px-3 py-1 bg-primary/5 border border-primary/20 rounded-2xl flex-shrink-0">
                  ...
                  <button aria-label="Cerrar" type="button" onClick={() => setShowAddCategory(false)} className="text-slate-400">
                ...
                <button
                  ...
                  className="btn-press whitespace-nowrap px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border-2 border-dashed border-primary/30 text-primary hover:bg-primary/5 transition-all flex items-center gap-1 flex-shrink-0"
                >
      ...
      <Reveal delay={0.2}>
        {isAdmin && pendingVideos.length > 0 && (
          <div className="mx-4 sm:mx-6 mb-6">
            <button
              onClick={() => setShowAdminPanel(true)}
              className="btn-press w-full flex items-center justify-between p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl hover:bg-amber-500/15 transition-all"
            >
      ...
      <main className="px-4 sm:px-6">
        {isLoading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="skeleton rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          >
            ...
            {activeFilterCount > 0 && (
              <button type="button" onClick={() => setFilters(EMPTY_FILTERS)}
                className="btn-press px-6 py-3 bg-primary text-white rounded-2xl font-black text-sm uppercase tracking-widest transition-all hover:shadow-xl hover:shadow-primary/30">
                Limpiar filtros
              </button>
            )}
          </motion.div>
            ...
            {viewMode === 'grid' ? (
              ...
              >
                {filteredVideos.map((video) => (
                  <motion.div key={video.id} variants={staggerItem} {...liftCard} className="card-hover min-w-0">
                    <WorkoutVideoCard
                      video={video}
              ...
              >
                {filteredVideos.map((video) => (
                  <motion.div key={video.id} variants={staggerItem} {...liftCard} className="card-hover">
                    <WorkoutVideoRow
                      video={video}
                      category={categories.find((c) => c.id === video.category_id)}
                      isAdmin={!!isAdmin}
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300 flex flex-col max-h-[90vh]">
            ...
            <div className="relative aspect-video bg-black">
              ...
              <button type="button" onClick={() => setSelectedVideoDetails(null)}
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="flex justify-between items-start mb-4">
                ...
                <button type="button" onClick={() => setShowRoutineModal(true)}
                  className="btn-press bg-primary text-white p-3 rounded-2xl shadow-lg shadow-primary/20 hover:scale-105 transition-transform"
            ...
            <div className="p-5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 space-y-3">
                  ...
                  <div className="flex gap-2">
                    {getVideoStatus(selectedVideoDetails) === 'pending' && (
                      <button
                        ...
                        className="btn-press bg-emerald-500 text-white px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20">
                        Aprobar
                      </button>
                    )}
                    <button
                      onClick={() => { handleDeleteVideo(selectedVideoDetails.id); setSelectedVideoDetails(null); }}
                      className="btn-press bg-red-500 text-white px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-red-600 transition-all shadow-lg shadow-red-500/20">
                      Eliminar
                    </button>
                  </div>
                </div>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setSelectedVideoDetails(null)}
                  className="btn-press flex-1 py-3.5 rounded-2xl font-bold text-slate-600 dark:text-white border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-sm">
                  Cerrar
                </button>
                {selectedVideoDetails.video_url && (
                  <button aria-label="Reproducir"
                    onClick={() => setFullScreenVideoUrl(selectedVideoDetails.video_url!)}
                    className="btn-press flex-[2] bg-primary text-white py-3.5 rounded-2xl font-bold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-colors flex items-center justify-center gap-2 text-sm">
        ...
        <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex justify-between items-center mb-5">
              ...
              <button aria-label="Cerrar" type="button" onClick={() => setShowRoutineModal(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-2 mb-5 max-h-56 overflow-y-auto">
              {userRoutines.length === 0 ? (
                ...
                userRoutines.map((routine) => (
                  <button type="button" key={routine.id}
                    onClick={async () => {
                      try {
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="flex gap-2">
                ...
                <button aria-label="Agregar"
                  ...
                  className="btn-press bg-primary text-white p-2.5 rounded-xl hover:bg-primary/90 transition-colors">
                  <Plus className="w-5 h-5" />
                </button>
              </div>
            </div>
      ...
      {fullScreenVideoUrl && (
        <div className="fixed inset-0 z-[70] bg-black flex flex-col">
          <div className="p-4 flex justify-end absolute top-0 left-0 right-0 z-10">
            <button aria-label="Cerrar" type="button" onClick={() => setFullScreenVideoUrl(null)}
          ...
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="btn-press flex-1 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-white py-3 rounded-2xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors text-sm">
                Cancelar
              </button>
              <button
                onClick={confirmDialog.onConfirm}
                className="btn-press flex-1 bg-red-500 text-white py-3 rounded-2xl font-black hover:bg-red-600 transition-colors shadow-lg shadow-red-500/20 text-sm">
