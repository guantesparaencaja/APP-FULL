      ...
      <header className="px-6 py-4 flex items-center justify-between backdrop-blur-md bg-white/5 border-b border-white/10 z-10 shrink-0">
        <div className="flex items-center gap-4">
          <button aria-label="Volver"
            onClick={() => navigate(-1)}
            className="p-2 bg-white/5 rounded-xl border border-white/10 text-white hover:bg-white/10 transition-colors btn-press"
          >
          ...
          <div className="space-y-4 py-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className={`flex ${i % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
                <div className={`w-3/5 h-12 rounded-4xl skeleton ${i % 2 === 0 ? 'rounded-tr-lg' : 'rounded-tl-lg'}`} />
              </div>
            ))}
          </div>
        ) : messages.length === 0 ? (
        ...
        <AnimatePresence>
          {isSending && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 8 }}
              className="flex items-center gap-1.5 self-start px-4 py-3 rounded-4xl rounded-tl-lg bg-white/5 border border-white/10 shadow-xl backdrop-blur-md mt-2"
            >
              {[0, 1, 2].map((i) => (
                <motion.span
                  key={i}
      ...
      <div className="p-4 sm:p-6 bg-white/5 border-t border-white/10 backdrop-blur-xl shrink-0">
        ...
        >
          ...
          <motion.button
            ...
            className="w-12 h-12 flex items-center justify-center bg-primary text-white rounded-2xl shadow-lg shadow-primary/30 disabled:opacity-50 disabled:grayscale transition-all btn-press"
          >
