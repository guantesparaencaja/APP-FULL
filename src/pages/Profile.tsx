...
import { User, Settings, LogOut, Shield, ArrowLeft, UserPlus, Camera, Image as ImageIcon, Edit2, Check, X, Users, Lock, Trash2, Moon, Sun, Monitor, Flame, Award, CalendarCheck, TrendingUp, Bell, MessageSquare, CreditCard, Heart, ChevronRight, ToggleLeft, ToggleRight, Target, Video, Activity } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import React, { useState, useRef, useEffect, useCallback } from 'react';
import { storage, db, auth, functions } from '../lib/firebase';
import { ref, uploadBytesResumable, getDownloadURL, deleteObject } from 'firebase/storage';
import { collection, onSnapshot, doc, updateDoc, setDoc, serverTimestamp, deleteDoc, query, where, getDoc, getDocs, addDoc } from 'firebase/firestore';
...
export function Profile() {
  ...
  const [expandedAdminSections, setExpandedAdminSections] = useState<Set<string>>(new Set(['pagos']));
  ...
  useEffect(() => {
    ...
    if (user?.role === 'admin' || user?.email === 'hernandezkevin001998@gmail.com') {
      ...
      unsubPayments = onSnapshot(paymentsQ, (snapshot) => {
        ...
        setPendingPayments(sortedAll.filter(p => p.status === 'submitted' || p.status === 'pending_class_payment'));
    ...
    if (user) {
      ...
      unsubAttendance = onSnapshot(allQ, (snapshot) => {
        ...
        const upcoming = snapshot.docs
          ...
          .filter((b: any) => b.status === 'active' && b.date >= today.toISOString().split('T')[0])
  ...
  const requestNotificationPermission = async () => {
    ...
    if (user) {
      try {
        await updateDoc(doc(db, 'users', String(user.id)), { notifications_enabled: permission === 'granted' });
    ...
    if (permission === 'granted') {
      ...
      showAlert('Permiso Denegado', 'Habilita las notificaciones en la vista del sitio de tu navegador (el ícono de candado en la barra de URL) para recibir alertas.', 'error');
    }
  };
  ...
  const handleApprovePayment = async (payment: any) => {
    ...
    try {
      ...
      const finalApprovedPrice = editingPaymentId === payment.id ? editPriceForm.final_price : (payment.final_price || payment.amount || 0);
      const finalDiscountReason = editingPaymentId === payment.id ? editPriceForm.discount_reason : (payment.discount_reason || '');
      ...
      if ((payment.type === 'single_class' || payment.plan_name?.toLowerCase().includes('clase individual')) && payment.booking_id) {
        ...
        showAlert('Éxito', 'Pago de clase individual aprobado. La reserva fue confirmada.', 'success');
      } else {
  ...
  const generateGoogleCalendarUrl = (booking: any) => {
    try {
      ...
      const startTimeStr = booking.time.includes('-') ? booking.time.split('-')[0].trim() : booking.time;
      const endTimeStr = booking.time.includes('-') ? booking.time.split('-')[1].trim() : `${parseInt(startTimeStr.split(':')[0]) + 1}:${startTimeStr.split(':')[1] || '00'}`;
      ...
      const details = encodeURIComponent(`Hola ${user?.name || ''}, esta es tu clase reservada. ¡No faltes!`);
  ...
  const handleApproveStep = async (userId: string, step: number) => {
    try {
      ...
      showAlert('✅ Aprobado', `Paso ${step} aprobado exitosamente. El estudiante puede continuar.`, 'success');
    } catch (err) {
  ...
  const handleRejectStep = async (userId: string, step: number) => {
    try {
      ...
      const field = `step${step}_status`;
      await setDoc(doc(db, 'student_approvals', userId), {
        [field]: 'rejected',
        updated_at: new Date().toISOString(),
      }, { merge: true });
      showAlert('❌ Rechazado', `Paso ${step} rechazado. El estudiante recibirá la notificación.`, 'info');
    } catch (err) {
      handleFirestoreError(err, 'update', `student_approvals/${userId}`);
    }
  };

  const handleApproveComboMethod = async (evaluation: any, method: 'manillas' | 'combo' | 'contacto') => {
    try {
  ...
  const handleRejectComboMethod = async (evaluation: any, method: 'manillas' | 'combo' | 'contacto') => {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      // 1. Reautenticar al admin
      const credential = EmailAuthProvider.credential(auth.currentUser.email!, deleteAdminPassword);
      await reauthenticateWithCredential(auth.currentUser, credential);
      
      // 2. Soft delete en Firestore (isActive: false)
      const userRef = doc(db, 'users', userToDelete.id);
      await updateDoc(userRef, { isActive: false });
      
      // 3. Cancelar futuras clases
      const now = new Date();
      const todayStr = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`;
      const bookingsRef = collection(db, 'bookings');
      const q = query(bookingsRef, where('user_id', '==', userToDelete.id), where('status', 'in', ['active', 'waitlist']));
      const pendingSnap = await getDocs(q);
      
      for (const bDoc of pendingSnap.docs) {
        if (bDoc.data().date >= todayStr) {
          await updateDoc(bDoc.ref, { status: 'cancelled' });
        }
      }
      
      // 4. Llamar a Cloud Function para borrado definitivo
      const deleteUserSecure = httpsCallable(functions, 'deleteUserSecure');
      await deleteUserSecure({ uid: userToDelete.id });
      
      // 5. Limpiar storage (archivos opcionales)
      ...
      const approvalSnap = await getDoc(doc(db, 'student_approvals', userToDelete.id));
      if (approvalSnap.exists()) {
         ...
         await deleteDoc(doc(db, 'student_approvals', userToDelete.id)).catch(()=>null);
      }
      ...
      // Revert soft delete si falla
      if (userToDelete) {
        await updateDoc(doc(db, 'users', userToDelete.id), { isActive: true }).catch(()=>null);
      }
      showAlert('Error', error.code === 'auth/wrong-password' ? 'Contraseña incorrecta' : 'Error al eliminar usuario: ' + error.message, 'error');
    } finally {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/wrong-password' || error.code === 'auth/invalid-credential') {
        showAlert('Error', 'La contraseña actual es incorrecta.', 'error');
      } else if (error.code === 'auth/requires-recent-login') {
        showAlert('Sesión caducada', 'Por seguridad, cierra sesión y vuelve a ingresar para cambiar tu contraseña.', 'error');
      } else {
        showAlert('Error', 'No se pudo actualizar la contraseña: ' + (error.message || ''), 'error');
      }
    } finally {
  ...
  const [adminEditForm, setAdminEditForm] = useState({ name: '', weight: 0, height: 0, newPassword: '', plan_name: '', classes_remaining: 0, classes_per_month: 0 });
  ...
  const handleAdminSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (adminEditForm.newPassword && adminEditForm.newPassword.length >= 6) {
        ...
        await updateAdminUserPassword({ uid: adminEditUser.id, newPassword: adminEditForm.newPassword });
      }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, setter: (val: string) => void, isProfilePic = false, isBefore = false, isAfter = false) => {
    ...
    if (file) {
      ...
      const storageRef = ref(storage, `images/${user?.id}/${type}_${Date.now()}_${compressedFile.name}`);
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col items-center mb-12">
          ...
          {user.role === 'student' ? (
            <div className="w-64 h-64">
               <EvolvingAvatar 
                  ...
                  level={((user.classes_per_month || 0) >= 4) ? 
                    ((user.lives || 0) <= 1 ? 'thin' : (user.streak || 0) >= 5 ? 'strong' : 'normal') : 
                    'normal'
                  } 
               />
            </div>
        ...
        <AnimatePresence mode="wait">
          {isEditing ? (
            ...
            >
              {(user.name === 'HONORARIOS' || user.name === 'honorarios') ? 'Usuario' : (user.name || 'Usuario')}
            </motion.h2>
          )}
        </AnimatePresence>
        ...
        <motion.section variants={itemVariants} className="mb-12">
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            ...
            >
              ...
              <span className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">{user.steps || 0}</span>
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] mt-2">Pasos Hoy</span>
            </motion.div>
            <motion.div 
              whileHover={{ y: -5 }}
              className="glass-card p-6 rounded-[2.5rem] flex flex-col items-center justify-center text-center relative overflow-hidden group"
          ...
          >
            <div className="flex justify-between items-center mb-6">
              ...
              <span className={`${Math.min(100, Math.round(((user.streak || 0) / 7) * 100)) > 50 ? 'text-emerald-500' : 'text-orange-500'} font-black text-2xl`}>{Math.min(100, Math.round(((user.streak || 0) / 7) * 100))}%</span>
            </div>
          ...
          <motion.div 
            whileHover={{ y: -5 }}
            className="glass-card rounded-[2.5rem] p-8 mt-6"
          >
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-4">
                ...
                <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-tight">Próximas Clases y Sincronización</h4>
              </div>
            </div>
            
            {upcomingBookings.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400 text-center font-medium leading-relaxed py-4 opacity-70">
                No tienes reservas activas por delante. Cuando reserves, podrás sincronizarlas con Google Calendar.
              </p>
            ) : (
              <div className="space-y-3">
                {upcomingBookings.map(b => (
                  <div key={b.id} className="flex justify-between items-center bg-white/50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/50">
                    <div>
                      <p className="font-black text-slate-900 dark:text-white">{b.date || 'Sin fecha'}</p>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-3 gap-4 mb-6">
        <motion.div 
          whileHover={{ scale: 1.02 }}
          className="glass-card p-5 rounded-[2rem] flex flex-col items-center relative overflow-hidden group"
        >
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-black tracking-[0.2em] mt-2 text-center">Peso</span>
        </motion.div>
        <motion.div 
          whileHover={{ scale: 1.02 }}
          className="glass-card p-5 rounded-[2rem] flex flex-col items-center relative overflow-hidden group"
        >
          ...
          {isEditing ? (
            ...
            <span className="text-emerald-500 text-xl font-black tracking-tight">{user.height ? `${user.height}cm` : '—'}</span>
          )}
          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-black tracking-[0.2em] mt-2 text-center">Altura</span>
        </motion.div>
        <motion.div 
          whileHover={{ scale: 1.02 }}
          className="glass-card p-5 rounded-[2rem] flex flex-col items-center relative overflow-hidden group"
        >
          ...
          {isEditing ? (
            ...
            <span className="text-primary text-sm font-black tracking-tight text-center">{user.dominant_hand || 'Derecha'}</span>
          )}
          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-black tracking-[0.2em] mt-2 text-center">Mano Dom.</span>
        </motion.div>
      </motion.div>
            ...
            {isEditing ? (
              <div className="w-full space-y-6">
                <div className="space-y-4 pt-4 border-t border-white/10">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Mi Género</label>
                  <div className="grid grid-cols-2 gap-4">
                    <button 
                      ...
                      className={twMerge(
                        ...
                        editForm.gender === 'male' ? "bg-primary border-primary text-white shadow-xl shadow-primary/20" : "bg-white/5 border-white/10 text-slate-500 hover:border-white/20"
                      )}
                    >
                    ...
                    <button 
                      ...
                      className={twMerge(
                        "p-5 rounded-2xl border-2 transition-all font-black uppercase tracking-[0.2em] text-[10px] flex items-center justify-center gap-3",
                        editForm.gender === 'female' ? "bg-primary border-primary text-white shadow-xl shadow-primary/20" : "bg-white/5 border-white/10 text-slate-500 hover:border-white/20"
                      )}
                    >
                ...
                <div className="space-y-4 pt-4 border-t border-white/10">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Mi Objetivo</label>
                    ...
                    {([
                      ...
                      <button
                        ...
                        className={twMerge(
                          ...
                          editForm.fitnessGoal === goal.id ? "bg-emerald-500 border-emerald-500 text-white shadow-xl shadow-emerald-500/20" : "bg-white/5 border-white/10 text-slate-500 hover:border-white/20"
                        )}
                      >
              ...
              <span className="text-primary text-xl font-black uppercase tracking-tight">
                {user.fitnessGoal === 'bajar_peso' ? 'Bajar Peso' : 
                 user.fitnessGoal === 'mantener' ? 'Mantener' : 
                 user.fitnessGoal === 'aumentar' ? 'Aumentar Masa' : 'General'}
              </span>
            )}
            <span className="text-[11px] text-slate-500 dark:text-slate-400 uppercase font-black tracking-[0.2em] mt-3">Mi Objetivo Fitness</span>
          </div>
        </motion.div>
      </motion.div>
        ...
        <div className="glass-card rounded-[2.5rem] p-8 space-y-2">
          ...
          {isEditing && (
            <div className="pt-4 mt-6 border-t border-slate-200/50 dark:border-slate-800/50 space-y-3">
              <p className="text-slate-900 dark:text-white text-sm font-bold uppercase tracking-widest">Cambiar Contraseña</p>
              {/* Contraseña actual */}
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowCurrentPwd(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                  {showCurrentPwd ? <Lock className="w-4 h-4" /> : <Lock className="w-4 h-4 opacity-40" />}
                </button>
              </div>
              {/* Nueva contraseña */}
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowNewPwd(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                  {showNewPwd ? <Lock className="w-4 h-4" /> : <Lock className="w-4 h-4 opacity-40" />}
                </button>
              </div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowConfirmPwd(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                  {showConfirmPwd ? <Lock className="w-4 h-4" /> : <Lock className="w-4 h-4 opacity-40" />}
                </button>
              </div>
              {passwordForm.confirmPassword && passwordForm.newPassword !== passwordForm.confirmPassword && (
              ...
              <button
                ...
                disabled={isChangingPassword || passwordForm.newPassword.length < 6 || passwordForm.newPassword !== passwordForm.confirmPassword || !passwordForm.currentPassword}
      ...
      {user.role !== 'admin' && (
        <motion.section variants={itemVariants} className="mb-12">
          <div className="flex items-center gap-4 mb-6">
            ...
            <h3 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white">Proceso de Licencia</h3>
          </div>
          <button
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-slate-500 mt-0.5">Video · Manillas · Contacto · Combo Presencial</p>
              </div>
            </div>
      ...
      {(user.role === 'admin' || user.email === 'hernandezkevin001998@gmail.com') && (
        <motion.div variants={itemVariants} className="space-y-12">
          <section>
            <button
              onClick={() => toggleAdminSection('pagos')}
            ...
            >
              ...
              <ChevronRight className={`w-5 h-5 text-slate-400 transition-transform ${expandedAdminSections.has('pagos') ? 'rotate-90' : ''}`} />
            </button>
            
            {expandedAdminSections.has('pagos') && (
              <div className="space-y-8">
                <motion.div whileHover={{ y: -5 }} className="glass-card rounded-[2.5rem] p-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                  <h4 className="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.25em] flex items-center gap-2">
                    Pagos de Planes y Clases
                    {paymentTab === 'pendientes' && pendingPayments.length > 0 && (
                  ...
                  <div className="flex bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl">
                    <button
                      onClick={() => setPaymentTab('pendientes')}
                      className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${paymentTab === 'pendientes' ? 'bg-primary text-white shadow-md' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                    >
                      Pendientes
                    </button>
                    <button
                      onClick={() => setPaymentTab('historial')}
                      className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${paymentTab === 'historial' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                    >
                      Historial
                    </button>
                  </div>
                ...
                {paymentTab === 'pendientes' ? (
                      ...
                      <AnimatePresence mode="popLayout">
                        {pendingPayments.map(p => (
                          <motion.div 
                            ...
                            className="bg-white/40 dark:bg-slate-800/40 p-6 rounded-[2rem] border border-amber-500/50 shadow-inner group"
                          >
                            <div className="flex justify-between items-start mb-6">
                              <div>
                                <p className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">{p.user_name}</p>
                                <p className="text-xs text-amber-500 font-black uppercase tracking-widest mt-1">{p.plan_name}</p>
                              </div>
                              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{new Date(p.submitted_at || p.created_at).toLocaleDateString()}</span>
                            </div>
                            <div className="aspect-video w-full overflow-hidden rounded-2xl mb-6 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 relative group/img">
                              <img 
                                ...
                                onError={(e) => { (e.target as HTMLImageElement).src = 'https://firebasestorage.googleapis.com/v0/b/gpte007.firebasestorage.app/o/qr-pago.jpg?alt=media'; }}
                              />
                            ...
                            <div className="flex flex-col gap-4">
                              {editingPaymentId === p.id ? (
                                <div className="bg-slate-100 dark:bg-slate-900/50 p-4 rounded-xl space-y-3 border border-slate-200 dark:border-slate-800">
                                  <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Precio Final (con descuento)</label>
                                    <input 
                                      type="number"
                                      value={editPriceForm.final_price}
                                      onChange={e => setEditPriceForm({...editPriceForm, final_price: Number(e.target.value)})}
                                      className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm font-bold"
                                    />
                                  </div>
                                  <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Motivo del Descuento</label>
                                    <input 
                                      type="text"
                                      placeholder="Ej: Promo Temporada"
                                      value={editPriceForm.discount_reason}
                                      onChange={e => setEditPriceForm({...editPriceForm, discount_reason: e.target.value})}
                                      className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm font-bold"
                                    />
                                  </div>
                                  <div className="flex gap-2">
                                    <button onClick={() => setEditingPaymentId(null)} className="flex-1 bg-slate-200 dark:bg-slate-800 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest">Cancelar</button>
                                    <button onClick={() => handleApprovePayment(p)} className="flex-1 bg-emerald-500 text-white py-2 rounded-lg text-[10px] font-black uppercase tracking-widest">Aprobar con cambios</button>
                                  </div>
                                </div>
                              ) : (
                                <>
                                  <div className="flex flex-col gap-2 bg-slate-100 dark:bg-slate-900/50 p-3 rounded-xl">
                                    <div className="flex justify-between items-center">
                                      <span className="text-[10px] font-black text-slate-400 uppercase">Precio Plan:</span>
                                      <span className="text-sm font-black text-slate-900 dark:text-white">${(p.amount || 0).toLocaleString()}</span>
                                    </div>
                                    {p.discount_amount > 0 && (
                                      <div className="flex justify-between items-center text-emerald-500">
                                        <span className="text-[10px] font-black uppercase italic">Descuento Nuevo Estudiante:</span>
                                        <span className="text-sm font-black italic">-${(p.discount_amount || 0).toLocaleString()}</span>
                                      </div>
                                    )}
                                    <div className="flex justify-between items-center border-t border-slate-200 dark:border-slate-800 pt-2">
                                      ...
                                      <span className="text-lg font-black text-slate-900 dark:text-white">${(p.final_price || p.amount || 0).toLocaleString()}</span>
                                    </div>
                                    <button 
                                      onClick={() => {
                                        setEditingPaymentId(p.id);
                                        setEditPriceForm({ 
                                          final_price: p.final_price || p.amount || 0,
                  ...
                  // Pestaña Historial
                  allPayments.filter(p => String(p.status).includes('approved') || String(p.status).includes('rejected')).length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 opacity-40">
                      <CreditCard className="w-12 h-12 mb-4" />
                      <p className="text-sm font-bold uppercase tracking-widest">Sin historial de pagos</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                      {allPayments.filter(p => String(p.status).includes('approved') || String(p.status).includes('rejected')).slice(0, 50).map(p => (
                        <div key={p.id} className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/40 p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800">
                          <div className="flex items-center gap-4">
                            <img 
                              src={p.payment_proof_url} 
                              alt="Comprobante History" 
                              className="w-12 h-12 rounded-xl object-cover border border-slate-300 dark:border-slate-700 cursor-pointer hover:scale-110 transition-transform" 
                              onClick={() => window.open(p.payment_proof_url, '_blank')}
                              onError={(e) => { (e.target as HTMLImageElement).src = 'https://firebasestorage.googleapis.com/v0/b/gpte007.firebasestorage.app/o/qr-pago.jpg?alt=media'; }}
                            />
                            <div>
                              <p className="font-bold text-slate-900 dark:text-white">{p.user_name}</p>
                              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black">{p.plan_name}</p>
                            </div>
                          </div>
                          <div className="flex flex-col items-end gap-1">
                            <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${String(p.status).includes('approved') ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
                              {String(p.status).includes('approved') ? 'Aprobado' : 'Rechazado'}
                            </span>
                            <span className="text-[9px] text-slate-400 font-bold">{new Date(p.submitted_at || p.created_at).toLocaleDateString()}</span>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <section>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white">Aprobaciones de Licencia</h3>
              </div>
              <div className="flex items-center gap-2">
                <span className="bg-amber-500/10 text-amber-400 px-4 py-1.5 rounded-xl border border-amber-500/20 text-[11px] font-black tracking-widest uppercase">
                  {pendingApprovals.filter(a => a.step1_status === 'uploaded' || a.step2_status === 'uploaded' || a.step3_status === 'uploaded' || a.step4_status === 'uploaded').length} pendientes
                </span>
                <ChevronRight className={`w-5 h-5 text-slate-400 transition-transform ${expandedAdminSections.has('licencias') ? 'rotate-90' : ''}`} />
              </div>
            </button>
            {expandedAdminSections.has('licencias') && (
              ...
              {pendingApprovals.length === 0 ? (
                ...
                <div className="space-y-6">
                  ...
                  {pendingApprovals
                    .filter(a => a.step1_status === 'uploaded' || a.step2_status === 'uploaded' || a.step3_status === 'uploaded' || a.step4_status === 'uploaded')
                    .map((approval) => {
                    const stepLabels = ['Video de Presentación', 'Manillas', 'Contacto', 'Combo Presencial'];
                    const stepsData = [
                      { key: 'step1', status: approval.step1_status, videoUrl: approval.step1_video_url },
                      { key: 'step2', status: approval.step2_status },
                      { key: 'step3', status: approval.step3_status },
                      { key: 'step4', status: approval.step4_status },
                    ];
                    return (
                      <div key={approval.id} className="bg-white/40 dark:bg-slate-800/40 p-6 rounded-[2rem] border border-slate-200/50 dark:border-slate-700/50">
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            <p className="font-black text-slate-900 dark:text-white text-lg">{approval.user_name || approval.id}</p>
                            <p className="text-xs text-primary font-bold">{approval.user_email}</p>
                          </div>
                          <div className="flex gap-1">
                            {stepsData.map((s, i) => (
                              <div key={i} className={`w-4 h-4 rounded-full border ${
                                s.status === 'approved' ? 'bg-emerald-500 border-emerald-500' :
                                s.status === 'uploaded' || s.status === 'pending' ? 'bg-amber-500 border-amber-500' :
                                s.status === 'rejected' ? 'bg-red-500 border-red-500' : 'bg-slate-700 border-slate-600'
                              }`} />
                            ))}
                          </div>
                        </div>
                        <div className="space-y-2">
                          {stepsData.map((step, i) => {
                            const stepNum = i + 1;
                            return (
                              <div key={step.key} className={`p-3 rounded-xl border text-sm flex items-center justify-between gap-2 ${
                                step.status === 'approved' ? 'border-emerald-500/20 bg-emerald-500/5' :
                                step.status === 'uploaded' ? 'border-amber-500/30 bg-amber-500/10' :
                                step.status === 'rejected' ? 'border-red-500/20 bg-red-500/5' :
                                'border-slate-700/50 bg-slate-900/30 opacity-40'
                              }`}>
                                <div className="flex items-center gap-2 min-w-0">
                                  <span className="text-[9px] font-black uppercase text-slate-400 shrink-0">P{stepNum}</span>
                                  <span className="font-bold text-white text-xs truncate">{stepLabels[i]}</span>
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                  {(step as any).videoUrl && (
                                    <a href={(step as any).videoUrl} target="_blank" rel="noopener noreferrer"
                                      className="px-2 py-1 bg-blue-500/10 text-blue-400 rounded-lg text-[9px] font-black uppercase border border-blue-500/20">
                                      📹 Ver
                                    </a>
                                  )}
                                  {(step.status === 'uploaded' || step.status === 'pending') && step.status !== 'approved' && step.status !== 'locked' && (
                                    <>
                                      <button onClick={() => handleApproveStep(approval.id, stepNum)}
                                        className="px-2 py-1 bg-emerald-500 text-white rounded-lg text-[9px] font-black uppercase">
                                        ✅
                                      </button>
                                      {stepNum === 1 && (
                                        <button onClick={() => handleRejectStep(approval.id, stepNum)}
                                  ...
                                  <span className={`text-[9px] font-black uppercase ${
                                    ...
                                    step.status === 'uploaded' ? 'text-amber-400' :
                                    step.status === 'rejected' ? 'text-red-400' : 'text-slate-600'
                                  }`}>
                                    {step.status === 'approved' ? '✅' : step.status === 'uploaded' ? '⏳' : step.status === 'rejected' ? '❌' : step.status === 'pending' ? '⌛' : '🔒'}
                                  </span>
                                </div>
                              </div>
                            );
          ...
          <section className="mb-12">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white">Evaluación de Técnicas</h3>
              </div>
              <div className="flex items-center gap-2">
                ...
                <ChevronRight className={`w-5 h-5 text-slate-400 transition-transform ${expandedAdminSections.has('tecnicas') ? 'rotate-90' : ''}`} />
              </div>
            </button>
            ...
            <div className="space-y-6">
              {allComboEvals.filter(e => e.combo_status === 'uploaded' || e.manillas_status === 'pending' || e.contacto_status === 'pending').length === 0 ? (
                <div className="text-center py-12 glass-card rounded-[2.5rem] border-dashed border-slate-700/50">
                  <p className="text-slate-500 text-xs font-black uppercase tracking-[0.2em] opacity-40">No hay evaluaciones de técnica pendientes</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-6">
                  {allComboEvals.filter(e => e.combo_status === 'uploaded' || e.manillas_status === 'pending' || e.contacto_status === 'pending').map((ev) => (
                    <div key={ev.id} className="glass-card rounded-[2.5rem] p-8 border border-slate-200/50 dark:border-slate-700/50 hover:border-indigo-500/30 transition-all">
                      <div className="flex flex-col md:flex-row gap-8">
                        {/* Student and Video Side */}
                        <div className="flex-1 space-y-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-[10px] font-black uppercase tracking-widest text-indigo-400 mb-1">Evaluación de Combo</p>
                              <h4 className="text-2xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">{ev.combo_name}</h4>
                              <p className="text-sm font-bold text-slate-500 dark:text-slate-400 mt-1">{ev.user_name}</p>
                            </div>
                          </div>

                          {ev.combo_video_url ? (
                            <div className="aspect-video bg-black rounded-3xl overflow-hidden border border-slate-800 shadow-2xl relative group">
                              <video src={ev.combo_video_url} controls className="w-full h-full" />
                              <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                                <p className="text-[9px] font-black text-white uppercase tracking-widest flex items-center gap-2">
                                  <Video className="w-3 h-3 text-indigo-400" /> Video del Estudiante
                                </p>
                              </div>
                            </div>
                          ) : (
                            <div className="aspect-video bg-slate-100 dark:bg-slate-900 rounded-3xl flex items-center justify-center border border-dashed border-slate-300 dark:border-slate-800">
                              <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Sin video adjunto</p>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex-1 flex flex-col gap-6">
                          {(['manillas', 'combo', 'contacto'] as const).map((method) => {
                            ...
                            return (
                              <div key={method} className={`p-5 rounded-[2rem] border transition-all ${
                                ...
                                <div className="flex items-center justify-between mb-4">
                                  <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-xl border ${
                                      ...
                                      {method === 'manillas' ? <Target className="w-4 h-4" /> : 
                                       method === 'combo' ? <Video className="w-4 h-4" /> : 
                                       <Flame className="w-4 h-4" />}
                                    </div>
                                    <span className="font-black uppercase tracking-widest text-xs text-slate-300">{method}</span>
                                  </div>
                                  <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md border ${
                                    status === 'approved' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                    status === 'uploaded' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                                    status === 'rejected' ? 'bg-red-500/10 text-red-500 border-red-500/20' :
                                    'bg-slate-800 text-slate-500 border-slate-700'
                                  }`}>
                                    {status === 'approved' ? 'Aprobado' : status === 'uploaded' ? 'En revisión' : status === 'rejected' ? 'Rechazado' : 'Pendiente'}
                                  </span>
                                </div>
                                ...
                                {status !== 'approved' && (
                                  <div className="space-y-4">
                                    <textarea
                                      ...
                                      onChange={(e) => setComboFeedback(prev => ({ ...prev, [feedbackKey]: e.target.value }))}
                                      placeholder={`Feedback para ${method}...`}
                                      className="w-full bg-slate-100 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3 text-xs text-slate-900 dark:text-slate-300 placeholder:text-slate-500 outline-none focus:border-indigo-500/50 h-20 resize-none transition-all"
                                    />
                                    <div className="flex gap-2">
                                      <button
          ...
          <section>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="bg-primary/10 text-primary px-4 py-1.5 rounded-xl border border-primary/20 text-[11px] font-black tracking-widest uppercase">{allUsers.length} Total</span>
                <ChevronRight className={`w-5 h-5 text-slate-400 transition-transform ${expandedAdminSections.has('usuarios') ? 'rotate-90' : ''}`} />
              </div>
            </button>
            {expandedAdminSections.has('usuarios') && (
              <>
                <motion.div whileHover={{ y: -5 }} className="glass-card rounded-[2.5rem] p-6 max-h-[400px] overflow-y-auto hide-scrollbar">
              {allUsers.length === 0 ? (
                        ...
                        {u.id !== user.id && u.email !== 'hernandezkevin001998@gmail.com' && (
                          <div className="flex gap-2">
                            <motion.button 
                              ...
                              onClick={() => {
                                setAdminEditUser(u);
                                setAdminEditForm({ name: u.name || '', weight: u.weight || 0, height: u.height || 0, newPassword: '', plan_name: u.plan_name || '', classes_remaining: u.classes_remaining || 0, classes_per_month: u.classes_per_month || 0 });
                              }}
          ...
          <section className="mb-12">
            <button
              onClick={() => toggleAdminSection('notificaciones')}
              className="flex items-center justify-between w-full mb-8 text-left"
            >
              <div className="flex items-center gap-4">
                <div className="p-3 bg-primary/10 rounded-2xl border border-primary/20">
              ...
              <ChevronRight className={`w-5 h-5 text-slate-400 transition-transform ${expandedAdminSections.has('notificaciones') ? 'rotate-90' : ''}`} />
            </button>
            {expandedAdminSections.has('notificaciones') && (
              <motion.div 
                whileHover={{ y: -5 }}
                className="glass-card rounded-[2.5rem] p-8"
              >
          ...
          <section className="mb-12">
            ...
            >
              ...
              <ChevronRight className={`w-5 h-5 text-slate-400 transition-transform ${expandedAdminSections.has('estudiantes') ? 'rotate-90' : ''}`} />
            </button>
            {expandedAdminSections.has('estudiantes') && (
              ...
              <div className="relative">
                <motion.button 
                  ...
                  className="w-full bg-primary/10 text-primary font-black py-5 rounded-[2rem] hover:bg-primary/20 transition-all border border-primary/30 items-center justify-center gap-4 uppercase tracking-[0.2em]"
                >
          ...
          <section className="mb-12">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white">Control de Secciones</h3>
              </div>
              <ChevronRight className={`w-5 h-5 text-slate-400 transition-transform ${expandedAdminSections.has('secciones') ? 'rotate-90' : ''}`} />
            </button>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`text-[11px] font-black uppercase tracking-widest px-3 py-1 rounded-lg ${
                          isOn ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-red-500/20 text-red-500'
                        }`}>
      ...
      >
        <form onSubmit={handleAdminSaveUser} className="p-4 space-y-4">
          <div>
            <label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Nombre</label>
            <input
              type="text"
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Peso (kg)</label>
              <input
                type="number"
                value={adminEditForm.weight}
                onChange={e => setAdminEditForm({...adminEditForm, weight: Number(e.target.value)})}
                className="w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-primary/50 outline-none"
              />
            </div>
            <div className="flex-1">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Altura (cm)</label>
              <input
                type="number"
                value={adminEditForm.height}
                onChange={e => setAdminEditForm({...adminEditForm, height: Number(e.target.value)})}
                className="w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-primary/50 outline-none"
              />
            </div>
          </div>
          <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
            <label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Plan Actual</label>
            <select
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block hover:text-primary transition-colors cursor-help" title={`Muestra el límite preestablecido de clases que proporciona el plan.\nClases consumidas: ${Math.max(0, adminEditForm.classes_per_month - adminEditForm.classes_remaining)}`}>
                Clases Totales (Base)
              </label>
              <input
                type="number"
                value={adminEditForm.classes_per_month}
                onChange={e => setAdminEditForm({...adminEditForm, classes_per_month: Number(e.target.value)})}
                className="w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-primary/50 outline-none"
              />
            </div>
            <div className="flex-1">
              <label className="text-xs font-black text-emerald-500 uppercase tracking-widest mb-1 block">Clases Restantes</label>
              <input
                type="number"
                value={adminEditForm.classes_remaining}
                onChange={e => setAdminEditForm({...adminEditForm, classes_remaining: Number(e.target.value)})}
          ...
          <div className="flex items-center gap-2 mb-4 bg-primary/10 p-2 rounded-lg">
            <span className="text-[10px] font-black text-primary uppercase tracking-widest">Consumidas:</span>
            <span className="text-xs font-bold text-slate-900 dark:text-white">
              {Math.max(0, adminEditForm.classes_per_month - adminEditForm.classes_remaining)} clases de {adminEditForm.classes_per_month}
            </span>
          </div>
          <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
            <label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Nueva Contraseña (Opcional)</label>
            <input
              type="password"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-400 text-center text-sm mb-6">
              Estás a punto de eliminar a <span className="text-white font-bold">{userToDelete.name}</span>. Esta acción no se puede deshacer. Por seguridad, ingresa tu contraseña de administrador:
            </p>
            <input
              type="password"
            ...
            <div className="flex gap-3">
              <button 
                onClick={() => { setUserToDelete(null); setDeleteAdminPassword(''); }}
