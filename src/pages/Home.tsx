    ...
    <div className="flex flex-col min-h-screen bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-sans pb-32">
        ...
        <div className="glass-card p-5 sm:p-8 rounded-3xl sm:rounded-[2.5rem] relative overflow-hidden">
            ...
            {isSpecialUser && (
              <div className="flex gap-2">
                <button aria-label="Agregar"
                  ...
                  className="px-3 py-2.5 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 rounded-xl transition-colors shrink-0 flex items-center gap-1.5 text-xs font-bold border border-emerald-500/20 btn-press"
                >
          ...
          {currentChallenge ? (
            ...
            <motion.div
              ...
              whileInView="show"
              viewport={{ once: true }}
              className="p-6 md:p-8 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/40 text-slate-400 dark:text-slate-500 transition-all"
            >
      ...
      >
        ...
        <motion.button
          variants={staggerItem}
          {...liftCard}
          onClick={() => isWorkoutsUnlocked && navigate('/boxing')}
          className={`relative overflow-hidden flex flex-col gap-4 sm:gap-6 glass-card-premium p-6 sm:p-10 rounded-4xl transition-all text-left group min-h-[200px] sm:min-h-[240px] ${!isWorkoutsUnlocked ? 'opacity-60 grayscale cursor-not-allowed' : 'hover:shadow-2xl hover:shadow-primary/15 neon-border-red'}`}
        >
          {!isWorkoutsUnlocked && (
            <div className="absolute inset-0 bg-slate-950/20 z-20 flex items-center justify-center backdrop-blur-[2px]">
              <Lock className="w-10 h-10 sm:w-12 sm:h-12 text-white/50" />
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 mt-2 sm:mt-4 font-medium leading-relaxed">
              {isWorkoutsUnlocked ? 'Planes de entrenamiento con videos.' : 'Sección Bloqueada'}
            </p>
          </div>
        </motion.button>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex justify-end -mb-6 relative z-20">
          <button
            ...
            className="flex items-center gap-2 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 px-4 py-2 rounded-2xl text-xs font-bold transition-all border border-slate-200 dark:border-slate-700 shadow-lg btn-press"
