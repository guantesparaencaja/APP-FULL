    ...
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 p-4 pb-32">
      ...
      <div className="dark">
        <PageHeader
          ...
          subtitle="Módulo Técnico Elite · Plan de Estudios 2026"
          right={
            <div className="flex items-center gap-2">
              <button aria-label="Volver"
                onClick={() => navigate('/saberes')}
                className="btn-press w-12 h-12 flex items-center justify-center bg-slate-900 rounded-2xl border border-slate-800 text-slate-400 hover:text-white transition-all"
              >
              ...
              {user?.role === 'admin' ? (
                <button aria-label="Ajustes"
                  ...
                  className="btn-press w-12 h-12 flex items-center justify-center bg-primary/20 rounded-2xl border border-primary/30 text-primary hover:bg-primary/30 transition-all"
                >
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="btn-press w-full flex items-center gap-5 p-6 text-left hover:bg-slate-800/50 transition-colors"
                >
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div
                      ...
                      className="px-6 pb-6 pt-2 border-t border-slate-800 space-y-4 overflow-hidden"
                    >
                    {moduleVideos.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {moduleVideos.map(video => (
                          <motion.button aria-label="Reproducir"
                            key={video.id}
        ...
        {selectedVideo && (
          ...
          >
            <motion.div
              ...
              transition={spring}
              className="max-w-3xl w-full bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl"
            >
              <div className="p-6 border-b border-slate-800 flex justify-between items-center">
                ...
                <button aria-label="Cerrar"
                  ...
                  className="btn-press p-3 bg-slate-800 rounded-2xl text-slate-400 hover:text-white transition-all"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
