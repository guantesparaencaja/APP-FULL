...
const RESTRICTED_CLASS_ROUTES = [
  '/boxing', '/saberes', '/workouts', '/meals', '/store', '/chat', '/timer',
];
...
function ProtectedRoute({
  ...
  useEffect(() => {
    ...
    // ── Bloqueo por cantidad de clases: 1-3 clases = acceso limitado,
    // 4+ clases = acceso completo a todas las secciones.
    const classes = user.classes_per_month || 0;
    const hasFullAccess = classes >= 4;
    if (!hasFullAccess) {
      const isRestricted = RESTRICTED_CLASS_ROUTES.some((r) => pathname.startsWith(r));
      if (isRestricted) {
        navigate('/');
        return;
      }
          ...
          <Suspense fallback={<LoadingFallback />}>
            <main>
              ...
              <Route element={<Layout />}>
                ...
                <Route path="/boxing" element={<ProtectedRoute><Boxing /></ProtectedRoute>} />
              </Route>
            </Routes>
          </main>
          </Suspense>
