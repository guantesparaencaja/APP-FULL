...
/** Escapa todo campo de usuario para prevenir inyección HTML en los correos. */
  ...
  return wrap(`
    <div style="${HEADER}">
      ...
      <h1 style="margin:0; color:#ffffff; font-size:28px; font-weight:800; letter-spacing:-0.5px;">
        ¡Bienvenido al equipo, ${escapeHtml(nombre)}!
      </h1>
    ...
    <div style="${BODY}">
      ...
      <p style="color:#cbd5e1; line-height:1.7; font-size:15px;">
        Hola <strong style="color:#f1f5f9;">${escapeHtml(nombre)}</strong>, nos alegra mucho tenerte en la familia GPTE.
  ...
  return wrap(`
    ...
    <div style="${BODY}">
      <p style="color:#cbd5e1; font-size:15px; line-height:1.7;">
        Hola <strong style="color:#f1f5f9;">${escapeHtml(data.nombre)}</strong>, tu reserva ha sido confirmada. 
      ...
      <div style="${INFO_ROW}">
        ...
        <div>
          ...
          <div style="color:#f1f5f9; font-weight:600; font-size:15px;">${escapeHtml(data.clase)}</div>
        </div>
      </div>
      <div style="${INFO_ROW}">
        <span style="font-size:20px;">📆</span>
        <div>
          ...
          <div style="color:#f1f5f9; font-weight:600; font-size:15px;">${escapeHtml(data.fecha)}</div>
        </div>
      </div>
      <div style="${INFO_ROW}">
        <span style="font-size:20px;">🕐</span>
        <div>
          ...
          <div style="color:#f1f5f9; font-weight:600; font-size:15px;">${escapeHtml(data.hora)}</div>
        </div>
      </div>
      <div style="${INFO_ROW}">
        ...
        <div>
          ...
          <div style="color:#f1f5f9; font-weight:600; font-size:15px;">${escapeHtml(data.tipo) || 'Clase Grupal'}</div>
        </div>
      </div>
      <hr style="${DIVIDER}">
...
export function templateClassCancel(data: {
  ...
  return wrap(`
    ...
    <div style="${BODY}">
      <p style="color:#cbd5e1; font-size:15px; line-height:1.7;">
        Hola <strong style="color:#f1f5f9;">${escapeHtml(data.nombre)}</strong>, lamentamos informarte que la siguiente 
      ...
      <div style="${INFO_ROW}">
        ...
        <div>
          ...
          <div style="color:#f1f5f9; font-weight:600; font-size:15px;">${escapeHtml(data.motivo)}</div>
        </div>
      </div>
      ` : ''}
      <hr style="${DIVIDER}">
  ...
  return wrap(`
    <div style="background: linear-gradient(135deg, #7c3aed 0%, #f97316 100%); padding: 40px 40px; text-align: center;">
      ...
      <h1 style="margin:0; color:#ffffff; font-size:30px; font-weight:800; letter-spacing:-0.5px;">
        ¡Feliz Cumpleaños, ${escapeHtml(nombre)}!
      </h1>
...
export function templateLateCancel(data: {
  ...
  return wrap(`
    ...
    <div style="${BODY}">
      <p style="color:#cbd5e1; font-size:15px; line-height:1.7;">
        Hola <strong style="color:#f1f5f9;">${escapeHtml(data.nombre)}</strong>, hemos detectado que la siguiente clase 
      ...
      <div style="${INFO_ROW}">
        ...
        <div>
          ...
          <div style="color:#f1f5f9; font-weight:600;">${escapeHtml(data.clase)}</div>
        </div>
      </div>
      <div style="${INFO_ROW}">
        <span style="font-size:20px;">📆</span>
        <div>
          ...
          <div style="color:#f1f5f9; font-weight:600;">${escapeHtml(data.fecha)} — ${escapeHtml(data.hora)}</div>
        </div>
      </div>
      <hr style="${DIVIDER}">
