...
export async function uploadVideo({
  ...
  // Plan gratuito de Supabase: máx. 50 MB por archivo. Validar antes de subir.
  const MAX_FREE_UPLOAD_MB = 50;
  const maxBytes = MAX_FREE_UPLOAD_MB * 1024 * 1024;
  if (file.size > maxBytes) {
    throw new Error(
      `El video pesa ${(file.size / 1024 / 1024).toFixed(1)} MB y supera el límite de ${MAX_FREE_UPLOAD_MB} MB del plan gratuito de Supabase. Compresión sugerida: H.264, resolución 720p, bitrate ~1.5 Mbps.`
    );
  }
