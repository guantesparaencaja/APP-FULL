...
export function FitnessGoalModal() {
  ...
  const colors = {
    ...
    mantener: {
      button: 'bg-emerald-500/5 border-emerald-500/20 hover:bg-emerald-500/10 hover:border-emerald-500/40',
      icon: 'bg-emerald-500/10 text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white'
    },
    aumentar: {
      button: 'bg-orange-500/5 border-orange-500/20 hover:bg-orange-500/10 hover:border-orange-500/40',
  ...
  const renderOption = (
    ...
    >
      <div className={`p-4 rounded-2xl transition-colors ${colors[value].icon}`}>
        {icon}
      </div>
      <div>
        <h4 className="font-black text-slate-900 dark:text-white uppercase tracking-tight text-lg">{title}</h4>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
        ...
        >
          ...
          <div className="space-y-4">
            {renderOption('bajar_peso', <TrendingDown className="w-6 h-6" />, 'Bajar de Peso', 'Enfocado en cardio y quema de calorías.')}
            {renderOption('mantener', <Scale className="w-6 h-6" />, 'Mantenerme', 'Equilibrio entre cardio y fuerza.')}
            {renderOption('aumentar', <TrendingUp className="w-6 h-6" />, 'Aumentar Masa', 'Rutinas enfocadas en hipertrofia y fuerza.')}
          </div>
