        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {routine.toolsUsed.map((tool) => (
              <span
                key={tool}
                className="bg-slate-800 text-slate-300 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border border-slate-700"
              >
                {translateTool(tool)}
              </span>
            ))}
        ...
        <div>
          ...
          <div className="space-y-3">
            {routine.exercises.map((ex, index) => (
              <div
                key={ex.id}
                className="flex items-center gap-4 bg-slate-950/50 p-3 rounded-2xl border border-slate-800/50"
              >
