...
import { Home, Target, Dumbbell, Calendar, User, Users, MessageSquare, Menu, X, ChevronRight, Shield, Bell, Apple } from 'lucide-react';
...
export function Layout() {
  ...
  const visibleNavItems = navItems.filter(item => {
    ...
    if (item.id === 'technique') return isSpecialUser || appSettings.technique_unlocked || hasFullAccess;
    if (item.id === 'workouts') return isSpecialUser || appSettings.workouts_unlocked || hasFullAccess;
  ...
  return (
    ...
    >
      ...
      <header className="fixed top-0 left-0 right-0 z-[60] px-4 py-3 flex items-center justify-between backdrop-blur-md bg-background-light/70 dark:bg-background-dark/70 border-b border-white/10 dark:border-slate-800/50">
        ...
        {/* RIGHT: branding */}
        <div className="flex items-center gap-2">
          <span className="font-black italic uppercase tracking-tighter text-lg">GPTE</span>
            ...
            >
              <div className="flex items-center justify-between mb-12">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center overflow-hidden shadow-inner">
                    {user?.profile_pic ? (
                      <img src={user.profile_pic} alt="Perfil" className="w-full h-full object-cover" />
                    ) : (
                      <User className="w-8 h-8 text-primary" />
                    )}
                  </div>
                  <div className="min-w-0">
                    <h3 className="font-black text-lg tracking-tight truncate max-w-[140px] dark:text-white">{user?.name || 'Usuario'}</h3>
                    <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">{user?.plan_name || 'Miembro'}</p>
                  </div>
                </div>
                <button 
                  onClick={() => setIsMenuOpen(false)} 
                  ...
                  return (
                    <motion.div key={item.path} variants={itemVariants}>
                      <Link
                        ...
                        onClick={() => setIsMenuOpen(false)}
                        className={twMerge(
                          "flex items-center justify-between p-4 rounded-[1.5rem] transition-all duration-300 group relative overflow-hidden",
                          isActive 
                      ...
                      >
                        <div className="flex items-center gap-4 relative z-10">
                          <div className={twMerge(
                            ...
                            isActive ? "bg-white/20" : "bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
                          )}>
                            <Icon className={clsx("w-5 h-5", isActive ? "stroke-[2.5px]" : "stroke-[2px]")} />
                          </div>
                          <span className="font-black uppercase italic tracking-tighter text-base">{item.label}</span>
                        </div>
                        <ChevronRight className={clsx("w-4 h-4 transition-transform group-hover:translate-x-1 relative z-10", isActive ? "opacity-100" : "opacity-30")} />
                        
                        {isActive && (
                          <motion.div 
                            layoutId="activeGlow" 
                            className="absolute inset-0 bg-gradient-to-r from-primary/0 via-white/5 to-white/10" 
                          />
                        )}
                      </Link>
                    </motion.div>
              ...
              <div className="mt-auto pt-8 border-t border-slate-100 dark:border-slate-800/50 space-y-4">
                <a 
                  ...
                  className="w-full flex items-center justify-center gap-3 bg-[#25D366] text-white p-4 rounded-[1.5rem] font-black uppercase tracking-widest shadow-lg shadow-[#25D366]/20 transition-all hover:scale-[1.02] active:scale-95 text-xs"
                >
                  <MessageSquare className="w-5 h-5" />
                  Soporte WhatsApp
                </a>
                <p className="text-[9px] font-bold text-slate-400 text-center uppercase tracking-[0.3em] opacity-50">Academia Guantes Para Encajar</p>
              </div>
            </motion.aside>
          </>
        )}
