...
export function OnboardingModal() {
  ...
  // Guard: si no hay usuario autenticado todavía, no renderizar nada
  if (!user) return null;
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="bg-slate-900 w-full max-w-md rounded-2xl border border-slate-700 p-6 shadow-2xl overflow-y-auto max-h-[85vh] pb-12">
        ...
        <p className="text-slate-400 mb-6 text-sm">
          Para continuar, necesitamos configurar tu perfil.
        </p>
        ...
        {step === 1 && (
          <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-right-4">
            ...
            <input
              ...
              className="bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-white focus:border-primary outline-none"
            />
            <input
              type="text"
        ...
        {step === 2 && (
          <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-right-4">
            ...
            {[
              'Boxeo para saber defenderme',
              'Boxeo para bajar peso o mantener saludable',
              'Boxeo competitivo',
            ].map((goal) => (
              <button
                key={goal}
        ...
        {step === 3 && (
          <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-right-4">
            ...
            {[
              'Bajar peso',
              'Mantener peso pero ser más resistente y fuerte',
              'Tener más masa muscular pero ser rápido, ágil y poderos@',
            ].map((goal) => (
              <button
                key={goal}
            ...
            {formData.fitness_goal === 'Bajar peso' && (
              <div className="mt-2 p-4 bg-slate-800/50 rounded-xl border border-slate-700">
                <p className="text-sm text-slate-300 mb-3">
                  ¿En cuánto tiempo quieres llegar a tu meta?
                </p>
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button
                ...
                className="flex-1 bg-emerald-500 text-white font-bold py-3 rounded-xl hover:bg-emerald-600 transition-colors shadow-[0_0_15px_rgba(16,185,129,0.4)]"
              >
                Finalizar
              </button>
            </div>
