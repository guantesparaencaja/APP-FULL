    ...
    <div className="relative w-full max-w-[340px] aspect-square mx-auto flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className={twMerge(
          ...
          isStrong ? 'bg-gradient-to-tr from-yellow-500 via-orange-600 to-red-500' : 
          isThin ? 'bg-slate-400' : 'bg-primary'
        )}
      />
      ...
      >
        <div className="relative group">
          ...
          <div className="flex items-center gap-4 bg-slate-900/90 border border-white/10 px-8 py-3.5 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-3xl overflow-hidden">
            ...
            <div className={twMerge(
              ...
              isStrong ? 'bg-yellow-400 shadow-yellow-400/60 animate-pulse' : 
              isThin ? 'bg-slate-500' : 'bg-primary shadow-primary/50'
            )} />
          ...
          <div className="mt-2 w-full h-1.5 bg-slate-800 rounded-full overflow-hidden border border-white/5">
            <motion.div 
              ...
              className={twMerge(
                ...
                isStrong ? "bg-gradient-to-r from-yellow-500 to-orange-500" :
                isThin ? "bg-slate-600" : "bg-primary"
              )}
            />
          </div>
        </div>
        ...
        >
          {isStrong ? 'La disciplina vence al talento' : isThin ? 'El descanso es parte del éxito' : 'Enfócate en la técnica'}
        </motion.span>
      </motion.div>
    </div>
  );
