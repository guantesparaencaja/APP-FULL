...
export const onVideoCreated = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            // ── SSRF guard: solo URLs https públicas, nunca IPs privadas ──
            const parsed = new URL(data.url);
            if (parsed.protocol !== "https:") {
                console.error("URL rechazada: solo se permiten URLs https.");
                return;
            }
            ...
            if (
                ...
                console.error("URL rechazada: host local o interno.");
                return;
            }
            // IPv4 privadas/reservadas (10.x, 127.x, 169.254.x, 172.16-31.x, 192.168.x, 0.0.0.0)
            if (/^\d{1,3}(\.\d{1,3}){3}$/.test(host)) {
                ...
                if (
                    ...
                    console.error("URL rechazada: IP privada/reservada.");
                    return;
                }
            }
            ...
            if (!response.ok) {
                console.error(`Respuesta no OK al descargar video: ${response.status}`);
                return;
            }
            // Límite de 120 MB para el video (evita agotar memoria)
            const contentLength = Number(response.headers.get("content-length") || "0");
            if (contentLength > 120 * 1024 * 1024) {
                console.error("Video demasiado grande (>120MB).");
                return;
            }
            ...
            if (buffer.byteLength > 120 * 1024 * 1024) {
                console.error("Video descargado supera los 120MB.");
                return;
            }
...
async function queueMail(to: string, subject: string, html: string): Promise<void> {
    try {
...
export const onBookingCreated = functions.firestore
    ...
    .onCreate(async (snap) => {
        ...
        if (subject && html) await queueMail(d.user_email, subject, html);
    });
...
export const onBookingUpdated = functions.firestore
    ...
    .onUpdate(async (change) => {
        ...
        if (subject && html) await queueMail(after.user_email, subject, html);
    });

export const onBookingDeleted = functions.firestore
    .document("bookings/{bookingId}")
    .onDelete(async (snap) => {
        const d = snap.data();
        if (!d.user_email) return;
        if (d.status !== "active" && d.status !== "waitlist") return;
...
export const updateAdminUserPassword = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (callerData?.role !== "admin" && callerData?.email !== "guantesparaencajar@gmail.com") {
...
export const deleteUserSecure = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Debe iniciar sesión");
    }
    ...
    if (callerData?.role !== "admin" && callerData?.email !== "hernandezkevin001998@gmail.com") {
        throw new functions.https.HttpsError("permission-denied", "Solo administradores pueden realizar esta acción");
    }
